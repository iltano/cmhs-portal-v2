(function () {
  ...
  let view={x:30,y:40,z:.75},drag=null,toastTimer,saveTimer,hubXML=data.hub.xml,hubFilename=data.hub.filename||'main.mhc',hubActive=false,removedHubKeys=new Set(),customTemplates=[],workspaceTemplates=[],workspaceNetworkTemplates=[],persistOK=true;
  ...
  function renderCatalog(){
    ...
    if(tab==='networks'){
      ...
      if(!filtered.length&&!workspaceNetworkTemplates.some(n=>(n.name+' '+(n.description||'')).toLowerCase().includes(q)))list.append(el('p','empty-result','No matching networks. Import .mhn files, import a configuration, or create a new network.'));
    }else{
  ...
  function refreshWorkspaceLibrary(){
    workspaceTemplates=[];workspaceNetworkTemplates=[];const elementSeen=new Set(),networkSeen=new Set();networks.forEach(network=>{try{const doc=new NetworkDocument(network.id===activeId?model.toXML():network.xml,network.filename),networkXML=doc.toXML(),networkKey=`${doc.name}\u0000${networkXML}`;if(!networkSeen.has(networkKey)){networkSeen.add(networkKey);workspaceNetworkTemplates.push({id:`workspace-network:${workspaceNetworkTemplates.length}`,name:doc.name,filename:doc.filename,description:doc.description,nodeCount:doc.nodes.length,connectionCount:doc.edges.length,xml:networkXML});}doc.nodes.forEach(node=>{const xml=serialize(node),key=`${node.tagName}\u0000${node.getAttribute('typename')}\u0000${xml}`;if(elementSeen.has(key))return;elementSeen.add(key);workspaceTemplates.push({id:`workspace:${workspaceTemplates.length}`,network:doc.name,name:node.getAttribute('name'),role:node.tagName,typename:node.getAttribute('typename'),xml});});}catch{/* Invalid imports are never added to the reusable library. */}});store.put('workspace-templates',workspaceTemplates);store.put('workspace-network-templates',workspaceNetworkTemplates);renderCatalog();toast(`Updated workspace library with ${workspaceNetworkTemplates.length} network template${workspaceNetworkTemplates.length===1?'':'s'} and ${workspaceTemplates.length} element configuration${workspaceTemplates.length===1?'':'s'}.`);
  }
  ...
  customTemplates=store.get('templates',[]);workspaceTemplates=store.get('workspace-templates',[]);workspaceNetworkTemplates=store.get('workspace-network-templates',[]);const hubState=store.get('hub-state',null);if(hubState?.xml){hubXML=hubState.xml;hubFilename=hubState.filename||'main.mhc';hubActive=!!hubState.active;removedHubKeys=new Set(hubState.removed||[]);}else hubXML=store.get('hub',data.hub.xml);
